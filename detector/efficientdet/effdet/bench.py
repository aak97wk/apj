...
def _post_process(config, cls_outputs, box_outputs):
    ...
    cls_outputs_all = jt.contrib.concat([cls_outputs[level].permute(0, 2, 3, 1).reshape([batch_size, (- 1), config.num_classes]) for level in range(config.num_levels)], dim=1)
    box_outputs_all = jt.contrib.concat([box_outputs[level].permute(0, 2, 3, 1).reshape([batch_size, (- 1), 4]) for level in range(config.num_levels)], dim=1)
    (_, cls_topk_indices_all) = jt.topk(cls_outputs_all.reshape((batch_size, (- 1))), MAX_DETECTION_POINTS, dim=1)
    indices_all = (cls_topk_indices_all // config.num_classes)
    classes_all = (cls_topk_indices_all % config.num_classes)
    box_outputs_all_after_topk = jt.gather(box_outputs_all, 1, indices_all.unsqueeze(2).expand((- 1), (- 1), 4))
    cls_outputs_all_after_topk = jt.gather(cls_outputs_all, 1, indices_all.unsqueeze(2).expand((- 1), (- 1), config.num_classes))
    cls_outputs_all_after_topk = jt.gather(cls_outputs_all_after_topk, 2, classes_all.unsqueeze(2))
...
class DetBenchEval(nn.Module):
    ...
    def execute(self, x, image_scales):
        ...
        return jt.stack(batch_detections, dim=0)
...
class DetBenchTrain(nn.Module):
    ...
    def execute(self, x, gt_boxes, gt_labels):
        (class_out, box_out) = self.model(x)
        loss = None
        gcl = []
        gbl = []
