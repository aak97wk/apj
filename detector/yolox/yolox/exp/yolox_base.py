...
class Exp(BaseExp):
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False, cache_img=False):
        ...
        dataset = MosaicDetection(dataset, mosaic=(not no_aug), img_size=self.input_size, preproc=TrainTransform(max_labels=120, flip_prob=self.flip_prob, hsv_prob=self.hsv_prob), degrees=self.degrees, translate=self.translate, mosaic_scale=self.mosaic_scale, mixup_scale=self.mixup_scale, shear=self.shear, enable_mixup=self.enable_mixup, mosaic_prob=self.mosaic_prob, mixup_prob=self.mixup_prob)
    ...
    def preprocess(self, inputs, targets, tsize):
        ...
        if ((scale_x != 1) or (scale_y != 1)):
            inputs = nn.interpolate(inputs, size=tsize, mode='nearest', align_corners=False)
        ...
        if ('optimizer' not in self.__dict__):
            ...
            for (k, v) in self.model.named_modules():
                ...
                if (isinstance(v, nn.BatchNorm2d) or ('bn' in k)):
                    ...
                    pg1.append(v.weight)
            optimizer = jt.optim.SGD(pg0, lr=lr, momentum=self.momentum, nesterov=True)
    ...
    def get_eval_loader(self, batch_size, is_distributed, testdev=False, legacy=False):
        ...
        valdataset = COCODataset(data_dir=self.data_dir, json_file=(self.val_ann if (not testdev) else self.test_ann), name=('val2017' if (not testdev) else 'test2017'), img_size=self.test_size, preproc=ValTransform(legacy=legacy))
        if is_distributed:
            batch_size = (batch_size // dist.get_world_size())
            sampler = jt.utils.data.distributed.DistributedSampler(valdataset, shuffle=False)
        else:
            sampler = jt.utils.data.SequentialSampler(valdataset)
        dataloader_kwargs = {'num_workers': self.data_num_workers, 'pin_memory': True, 'sampler': sampler}
        dataloader_kwargs['batch_size'] = batch_size
        val_loader = jt.utils.data.DataLoader(valdataset, **dataloader_kwargs)
        return val_loader
