...
class Predictor(object):
    ...
    def __init__(self, model, exp, cls_names=COCO_CLASSES, trt_file=None, decoder=None, device='cpu', fp16=False, legacy=False):
        ...
        if (trt_file is not None):
            model_trt = TRTModule()
            model_trt.load_parameters(jt.load(trt_file))
            x = jt.ones((1, 3, exp.test_size[0], exp.test_size[1]))
            self.model(x)
    ...
    def inference(self, img):
        ...
        with jt.no_grad():
            ...
            if (self.decoder is not None):
                outputs = self.decoder(outputs, dtype=outputs.dtype)
