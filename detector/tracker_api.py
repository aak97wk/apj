...
class Tracker(BaseDetector):
    ...
    def __init__(self, cfg, opt=None):
        ...
        self.max_time_lost = cfg.get('BUFFER_SIZE', 30)
        self.model = None
    ...
    def load_model(self):
        ...
        self.model.load_parameters(jt.load(self.model_weights)['model'], strict=False)
    ...
    def images_detection(self, imgs, orig_dim_list):
        ...
        for image_i in range(len(imgs)):
            ...
            if (dets[image_i] is not None):
                ...
                'Detections'
                detections = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f.numpy(), 30) for (tlbrs, f) in zip(det_i[:, :5], det_i[:, (- self.emb_dim):])]
            else:
                detections = []
            ...
            for t in self.tracked_stracks:
                ...
                if ((tlwh[2] * tlwh[3]) > self.tracker_opt.min_box_area):
                    res = jt.array([image_i, tlbr[0], tlbr[1], tlbr[2], tlbr[3], ts, tid])
                output_stracks.append(res)
        if (len(output_stracks) == 0):
            return 0
        return jt.stack(output_stracks)
