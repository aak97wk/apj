...
def arg_parse():
    ...
    parser.add_argument('--cfg', dest='cfgfile', help='Config file', default='cfg/yolov3-spp.cfg', type=str)
    parser.add_argument('--weights', dest='weightsfile', help='weightsfile', default='yolov3-spp.weights', type=str)
...
if (__name__ == '__main__'):
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            im_dim = jt.float32(dim).repeat(1, 2)
            if CUDA:
                img = img.half()
                im_dim = im_dim.half()
            ...
            scaling_factor = jt.min((inp_dim / im_dim), 1).view((- 1), 1)
            output[:, [1, 3]] -= ((inp_dim - (scaling_factor * im_dim[:, 0].view(((- 1), 1)))) / 2)
            output[:, [2, 4]] -= ((inp_dim - (scaling_factor * im_dim[:, 1].view(((- 1), 1)))) / 2)
            output[:, 1:5] /= scaling_factor
            for i in range(output.shape[0]):
                output[(i, [1, 3])] = jt.clamp(output[(i, [1, 3])], min_v=0.0, max_v=im_dim[(i, 0)])
                output[(i, [2, 4])] = jt.clamp(output[(i, [2, 4])], min_v=0.0, max_v=im_dim[(i, 1)])
            classes = load_classes('data/coco.names')
            colors = jt.load(open('pallete', 'rb'))
