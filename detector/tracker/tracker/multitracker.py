...
class JDETracker(object):
    ...
    def __init__(self, args, frame_rate=30):
        ...
        self.model.load_parameters(jt.load(args.weights)['model'], strict=False)
        # if args:
        #     if (len(args.gpus) > 1):
    ...
    def update(self, im_blob, img0):
        ...
        if (len(pred) > 0):
            dets = non_max_suppression(pred.unsqueeze(0), self.args.conf_thres, self.args.nms_thres)[0]
            scale_coords(self.args.img_size, dets[:, :4], img0.shape).round()
            'Detections'
            detections = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f.numpy(), 30) for (tlbrs, f) in zip(dets[:, :5], dets[:, (- self.model.emb_dim):])]
        else:
            detections = []
        t2 = time.time()
