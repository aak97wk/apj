...
def crop_with_factor(im, dest_size, factor=32, pad_val=0, basedon='min'):
    ...
    im_base = {'min': im_size_min, 'max': im_size_max, 'w': im.shape[1], 'h': im.shape[0]}
...
class PatchClassifier(object):
    ...
    def update(self, image):
        ...
        im_data = jt.array(im_croped).permute(2, 0, 1)
    ...
    def predict(self, rois):
        ...
        ratios = (areas / np.clip(ori_areas, a_min=0.0001, a_max=None))
