...
def save_net(fname, net, epoch=(- 1), optimizers=None, rm_prev_opt=False, max_n_ckpts=(- 1)):
    ...
    with h5py.File(fname, mode='w') as h5f:
        for (k, v) in net.state_dict().items():
            h5f.create_dataset(k, data=v.numpy())
        ...
        if rm_prev_opt:
            ...
            for filename in os.listdir(root):
                ...
                if (filename.endswith('.optimizer_state.pk') and (filename != state_file)):
                    ...
                    raise RuntimeError('original source: <os.remove(filename)>, remove is not supported in Jittor yet. We will appreciate it if you provide an implementation of remove and make pull request at https://github.com/Jittor/jittor.')
        if (max_n_ckpts > 0):
            root = os.path.split(fname)[0]
...
def load_net(fname, net, prefix='', load_state_dict=False):
    ...
    with h5py.File(fname, mode='r') as h5f:
        ...
        for (k, v) in net.state_dict().items():
            k = (prefix + k)
            if (k in h5f):
                param = jt.array(np.asarray(h5f[k]))
                if (v.shape != param.shape):
                    logger.warning('Inconsistent shape: {}, {}'.format(v.shape, param.shape))
                else:
                    v.copy_(param)
            else:
