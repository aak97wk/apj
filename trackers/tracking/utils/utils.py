...
def xyxy2xywh(x):
    y = (jt.zeros(x.shape) if (x.dtype is jtfloat32) else np.zeros(x.shape))
    y[:, 0] = ((x[:, 0] + x[:, 2]) / 2)
    y[:, 1] = ((x[:, 1] + x[:, 3]) / 2)
    y[:, 2] = (x[:, 2] - x[:, 0])
    y[:, 3] = (x[:, 3] - x[:, 1])
...
def build_targets_max(target, anchor_wh, nA, nC, nGh, nGw):
    ...
    for b in range(nB):
        ...
        gwh[:, 0] = (gwh[:, 0] * nGw)
        gwh[:, 1] = (gwh[:, 1] * nGh)
        gi = jtclamp(gxy[:, 0], min=0, max=(nGw - 1)).long()
        gj = jtclamp(gxy[:, 1], min=0, max=(nGh - 1)).long()
        box1 = gwh
        box2 = anchor_wh.unsqueeze(1)
        inter_area = jtmin(box1, box2).prod(dim=2)
        ...
        if (nTb > 1):
            ...
            first_unique = return_torch_unique_index(u, jtunique(u, dim=1))
