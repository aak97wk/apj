        ...
        for pid in range(len(tracked[imgname])):
            ...
            if (pose.shape[0] == 16):
                ...
                for (idx_c, color) in enumerate(colors):
                    plt.plot(np.clip(pose[(idx_c, 0)], 0, width), np.clip(pose[(idx_c, 1)], 0, height), marker='o', color=color, ms=((80 / alpha_ratio) * np.mean(pose[(idx_c, 2)])), markerfacecolor=(1, 1, 0, ((0.7 / alpha_ratio) * pose[(idx_c, 2)])))
                for idx in range(len(pairs)):
                    plt.plot(np.clip(pose[(pairs[idx], 0)], 0, width), np.clip(pose[(pairs[idx], 1)], 0, height), 'r-', color=cmap(tracked_id), linewidth=((60 / alpha_ratio) * np.mean(pose[(pairs[idx], 2)])), alpha=((0.6 / alpha_ratio) * np.mean(pose[(pairs[idx], 2)])))
...
if (__name__ == '__main__'):
    ...
    for (idx, frame_name) in enumerate(tqdm(frame_list[:(- 1)])):
        ...
        (match_indexes, match_scores) = best_matching_hungarian(all_cors, cur_all_pids, cur_all_pids_fff, track[next_frame_name], weights, weights_fff, num, mag)
