...
parser.add_argument('--nThreads', default=0, type=int, help='Number of data loading threads')
...
'----------------------------- Training options -----------------------------'
parser.add_argument('--sync', default=False, dest='sync', help='Use Sync Batchnorm', action='store_true')
parser.add_argument('--detector', dest='detector', help='detector name', default='yolo')
'----------------------------- Log options -----------------------------'
parser.add_argument('--board', default=True, dest='board', help='Logging with tensorboard', action='store_true')
parser.add_argument('--debug', default=False, dest='debug', help='Visualization debug', action='store_true')
parser.add_argument('--map', default=True, dest='map', help='Evaluate mAP per epoch', action='store_true')
...
opt.gpus = [i for i in range(jt.get_device_count())]
...
def epochInfo(self, set, idx, loss, acc):
    self.info('{set}-{idx:d} epoch | loss:{loss:.8f} | acc:{acc:.4f}'.format(set=set, idx=idx, loss=loss, acc=acc))
