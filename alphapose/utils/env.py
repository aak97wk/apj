...
def init_dist(opt):
    ...
    opt.ngpus_per_node = jt.get_device_count()
...
def _init_dist_slurm(opt, port=23348, **kwargs):
    ...
    dist.init_process_group(backend=opt.dist_backend, init_method=f'tcp://{node_list}:{port}', world_size=opt.world_size, rank=opt.rank)
    if (opt.rank == 0):
        opt.log = True
    else:
        opt.log = False
