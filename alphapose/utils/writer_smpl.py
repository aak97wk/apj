...
class DataWriterSMPL():
    ...
    def update(self):
        ...
        while True:
            (boxes, scores, ids, smpl_output, cropped_boxes, orig_img, im_name) = self.wait_and_get(self.result_queue)
            if (orig_img is None):
                if self.save_video:
                    stream.release()
            ...
            if ((boxes is None) or (len(boxes) == 0)):
                ...
                uv_29 = smpl_output['pred_uvd_jts'].reshape(((- 1), 29, 3))[:, :, :2]
                pred_xyz_jts_24 = smpl_output['pred_xyz_jts_24'].reshape(((- 1), 24, 3))
                ...
                boxes = boxes.jt.tolist()
                cropped_boxes = cropped_boxes.jt.tolist()
                _result = []
                for k in range(len(scores)):
                    _result.append({'keypoints': preds_img[k], 'pred_xyz_jts': pred_xyz_jts_24[k], 'kp_score': preds_scores[k], 'proposal_score': ((jt.mean(preds_scores[k]) + scores[k]) + (1.25 * max(preds_scores[k]))), 'bbox_score': scores[k], 'idx': ids[k].item(), 'box': [boxes[k][0], boxes[k][1], (boxes[k][2] - boxes[k][0]), (boxes[k][3] - boxes[k][1])], 'crop_box': [cropped_boxes[k][0], cropped_boxes[k][1], (cropped_boxes[k][2] - cropped_boxes[k][0]), (cropped_boxes[k][3] - cropped_boxes[k][1])]})
