...
from ..transforms import addDPG, affine_transform, flip_joints_3d, flip_thetas, flip_xyz_joints_3d, get_affine_transform, im_to_torch, batch_rodrigues_numpy, rotmat_to_quat_numpy, flip_twist, get_intrinsic_metrix
...
class SimpleTransform3DSMPL(object):
    ...
    def __init__(self, dataset, scale_factor, color_factor, occlusion, add_dpg, input_size, output_size, depth_dim, bbox_3d_shape, rot, sigma, train, loss_type='MSELoss', scale_mult=1.25, two_d=False):
        if two_d:
    ...
    def __call__(self, src, label):
        if self.two_d:
            ...
            if (self._train and ((np.sum(joints_vis_17[:, 0]) > self.num_joints_half_body) and (np.random.rand() < self.prob_half_body))):
                (c_half_body, s_half_body) = self.half_body_transform(gt_joints_17[:, :, 0], joints_vis_17)
                if ((c_half_body is not None) and (s_half_body is not None)):
                    (center, scale) = (c_half_body, s_half_body)
            if self._train:
                sf = self._scale_factor
            ...
            target_smpl_weight = jt.ones(1).float()
        ...
        img[2].div_(0.229)
        if self.two_d:
            output = {'type': '2d_data', 'image': img, 'target': jt.array(target).float(), 'target_weight': jt.array(target_weight).float(), 'trans_inv': jt.array(trans_inv).float(), 'intrinsic_param': jt.array(intrinsic_param).float(), 'joint_root': jt.array(joint_root).float(), 'depth_factor': jt.array(depth_factor).float(), 'bbox': jt.Var(bbox)}
        else:
            output = {'type': '3d_data_w_smpl', 'image': img, 'target_theta': jt.array(theta_quat).float(), 'target_theta_weight': jt.array(theta_24_weights).float(), 'target_beta': jt.array(beta).float(), 'target_smpl_weight': target_smpl_weight, 'target_uvd_29': jt.array(target_uvd_29).float(), 'target_xyz_24': jt.array(target_xyz_24).float(), 'target_weight_29': jt.array(target_weight_29).float(), 'target_weight_24': jt.array(target_weight_24).float(), 'target_xyz_17': jt.array(target_xyz_17).float(), 'target_weight_17': jt.array(target_weight_17).float(), 'trans_inv': jt.array(trans_inv).float(), 'intrinsic_param': jt.array(intrinsic_param).float(), 'joint_root': jt.array(joint_root).float(), 'depth_factor': jt.array(depth_factor).float(), 'bbox': jt.Var(bbox), 'target_twist': jt.array(twist_phi).float(), 'target_twist_weight': jt.array(twist_weight).float()}
        return output
