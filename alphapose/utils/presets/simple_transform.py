...
from ..transforms import addDPG, affine_transform, flip_joints_3d, get_affine_transform, im_to_torch
...
class SimpleTransform(object):
    ...
    def __init__(self, dataset, scale_factor, add_dpg, input_size, output_size, rot, sigma, train, gpu_device=None, loss_type='MSELoss'):
        ...
        if (platform.system() != 'Windows'):
            self.roi_align = ROIAlign(self._input_size,
                                      spatial_scale=1.
                                      # sample_num=(- 1)
                                      )
    ...
    def align_transform(self, image, boxes):
        ...
        cropped_img = self.roi_align(tensor_img.unsqueeze(0), new_boxes)
    ...
    def __call__(self, src, label):
        ...
        if (self._loss_type == 'Combined'):
            return (img, [jt.array(target_mse), jt.array(target_inter)], [jt.array(target_weight_mse), jt.array(target_weight_inter)], jt.Var(bbox))
        else:
            return (img, jt.array(target), jt.array(target_weight), jt.Var(bbox))
