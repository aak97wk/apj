...
def mask_cross_entropy(pred, target):
    return nn.binary_cross_entropy_with_logits(pred, target)[None]
...
def calc_integral_accuracy(preds, labels, label_masks, output_3d=False, norm_type='softmax'):
    ...
    if output_3d:
        labels = labels.numpy().reshape(preds.shape[0], num_joints, 3)
        label_masks = label_masks.numpy().reshape(preds.shape[0], num_joints, 3)
        ...
        labels = labels.numpy().reshape(preds.shape[0], num_joints, 2)
        label_masks = label_masks.numpy().reshape(preds.shape[0], num_joints, 2)
        labels[:, :, 0] = ((labels[:, :, 0] + 0.5) * hm_width)
        labels[:, :, 1] = ((labels[:, :, 1] + 0.5) * hm_height)
