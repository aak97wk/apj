...
class DWConvLayer(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, norm_layer, stride=1, bias=False):
        ...
        self.add_module('dwconv', nn.Conv(groups, groups, 3, stride=stride, padding=1, groups=groups, bias=bias))
...
class ConvLayer(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, norm_layer, kernel=3, stride=1, padding=0, bias=False):
        ...
        self.add_module('conv', nn.Conv(in_channels, out_ch, kernel, stride=stride, padding=pad, groups=groups, bias=bias))
...
class HarDBlock_v2(nn.Module):
    ...
    def __init__(self, in_channels, growth_rate, grmul, n_layers, norm_layer, dwconv=False):
        ...
        for i in range(n_layers):
            ...
            conv_layers_.append(nn.Conv(cur_ch, accum_out_ch, 3, stride=1, padding=1, bias=True))
    ...
    def execute(self, x):
        ...
        for i in range(len(self.conv_layers)):
            ...
            if (((i % 2) == 0) or (i == (len(self.conv_layers) - 1))):
                outs_.append(xin)
        out = jt.contrib.concat(outs_, dim=1)
        return out
...
class HarDNetBase(nn.Module):
    ...
    def __init__(self, arch, norm_layer, depth_wise=False):
        ...
        self.base.append(ConvLayer(in_channels=3, out_channels=first_ch[0], norm_layer=norm_layer, kernel=3, stride=2, bias=False))
        self.base.append(ConvLayer(first_ch[0], first_ch[1], norm_layer, kernel=second_kernel))
        self.base.append(nn.Pool(3, stride=2, padding=1, op='mean'))
        ch = first_ch[1]
        for i in range(blks):
            ...
            ch = ch_list[i]
            if (i == 0):
                self.base.append(nn.Pool(2, stride=2, ceil_mode=True, op='mean'))
            elif ((i != (blks - 1)) and (i != 1) and (i != 3)):
                self.base.append(nn.Pool(2, stride=2, op='mean'))
...
class TransitionUp(nn.Module):
    ...
    def execute(self, x, skip, concat=True):
        out = nn.interpolate(x, size=(skip.shape[2], skip.shape[3]), mode='nearest', align_corners=True)
        if concat:
            out = jt.contrib.concat([out, skip], dim=1)
        return out
...
class HarDNetPose(nn.Module):
    ...
    def __init__(self, norm_layer=nn.BatchNorm2d, **cfg):
        ...
        self.last_pool = nn.Pool(2, stride=2, op='mean')
        ...
        self.avg9x9 = nn.Pool((9, 9), stride=1, padding=(4, 4), op='mean')
        ...
        self.conv_out = nn.Sequential(nn.Conv(prev_ch, ch, 3, padding=1, bias=True), nn.ReLU(), nn.Conv(ch, out_channel, cfg['FINAL_CONV_KERNEL'], stride=1, padding=(cfg['FINAL_CONV_KERNEL'] // 2), bias=True))
    ...
    def execute(self, x):
        ...
        x = self.last_pool(x)
        x2 = self.avg9x9(x)
        x3 = (x / (x.sum(dim=(2, 3), keepdims=True) + 0.1))
        x = jt.contrib.concat([x, x2, x3], dim=1)
        x = self.last_blk(x)
        for i in range(3):
            skip_x = xs[(3 - i)]
        ...
        for i in range(3):
            if (self.SC[i] > 0):
                scs.insert(0, nn.interpolate(x_sc[i], size=(x.shape[2], x.shape[3]), mode='nearest', align_corners=True))
        x = jt.contrib.concat(scs, dim=1)
        x = self.conv_out(x)
        return x
