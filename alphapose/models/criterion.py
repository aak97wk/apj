...
class IngetralCoordinate():
    ' Symmetry integral regression function.\n    '
    AMPLITUDE = 2

    @staticmethod
    def execute(ctx, input):
        assert isinstance(input, jt.Var), 'IngetralCoordinate only takes input as jt.array'
        input_size = input.size()
        weight = jt.arange(input_size[(- 1)], dtype=input.dtype)
    ...
    def backward(ctx, grad_output):
        ...
        output_coord = output.sum(dim=2, keepdims=True)
        weight = weight[None, None, :].repeat(output_coord.shape[0], output_coord.shape[1], 1)
        weight_mask = jt.ones(weight.shape, dtype=grad_output.dtype)
        weight_mask[(weight < output_coord)] = (- 1)
        weight_mask[(output_coord.repeat(1, 1, weight.shape[(- 1)]) > ctx.input_size[(- 1)])] = 1
        weight_mask *= IngetralCoordinate.AMPLITUDE
        return grad_output * (weight_mask)
...
class L1JointRegression(nn.Module):
    ...
    def execute(self, preds, *args):
        ...
        (pred_jts, pred_scores) = _integral_tensor(preds, num_joints, self.output_3d, hm_width, hm_height, hm_depth, integral_operation=self.integral_operation, norm_type=self.norm_type)
...
def weighted_l1_loss(input, scores, target, weights, size_average):
    out = jt.abs((input - target))
    out = (out * weights)
    if size_average:
        return (out.sum() / len(input))
    else:
        return out.sum()
LOSS.register_module(jt.nn.MSELoss)
