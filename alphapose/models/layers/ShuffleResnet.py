    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=False, norm_layer=nn.BatchNorm2d, dcn=None):
        ...
        if (stride > 1):
            ...
            if ((not self.with_dcn) or fallback_on_stride):
                conv_layers.append(nn.Conv((planes * 4), planes, 3, stride=1, padding=1, bias=False))
            else:
                from .dcn import DCN
...
class ShuffleResnet(nn.Module):
    ...
    def make_layer(self, block, planes, blocks, stride=1, dcn=None):
        ...
        if ((stride != 1) or (self.inplanes != (planes * block.expansion))):
            downsample = nn.Sequential(nn.Conv(self.inplanes, (planes * block.expansion), 1, stride=stride, bias=False), self._norm_layer((planes * block.expansion), momentum=0.1))
        layers = []
