...
class FastPose_DUC_Dense(nn.Module):
    ...
    def __init__(self, norm_layer=nn.BatchNorm2d, **cfg):
        ...
        if (cfg['NUM_LAYERS'] == 152):
            ...
            x = models.resnet152(pretrained=True)
        elif (cfg['NUM_LAYERS'] == 101):
            ' Load pretrained model '
            x = models.resnet101(pretrained=True)
        ...
        self.conv_out_dense = nn.Conv(self.conv_dim, (self._preset_cfg['NUM_JOINTS_DENSE'] - self._preset_cfg['NUM_JOINTS']), 3, stride=1, padding=1)
    ...
    def execute(self, x):
        ...
        out = jt.contrib.concat((out, out_dense), dim=1)
        return out
