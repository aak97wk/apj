...
def deformable_col2im_coord(
    ...
    height_col, width_col, out0_p);
    """
    return jt.code(grad_offset_shape, data_offset.dtype, [data_col, data_im, data_offset], cuda_header=HEADER,
                   cuda_src=src)
...
def deform_conv_forward_cuda(input, weight, offset,
    ...
    offset = offset.view(
        (batchSize // im2col_step, im2col_step, deformable_group * 2 * kH * kW, outputHeight, outputWidth))
    ...
    output_buffer = output_buffer.view(
        (output_buffer.size(0), group, output_buffer.size(1) // group, output_buffer.size(2), output_buffer.size(3)))
    ...
    for elt in range(batchSize // im2col_step):
        ...
        for g in range(group):
            output_buffer[elt, g] = (
                        output_buffer[elt, g].flatten(1) + jt.matmul(weight[g].flatten(1), columns[g])).view_as(
                output_buffer[elt, g])
...
def deform_conv_backward_input_cuda(input, offset,
    ...
    for elt in range(batchSize // im2col_step):
        columns = columns.view((group, columns.size(0) // group, columns.size(1)))
        weight = weight.view((group, weight.size(0) // group, weight.size(1), weight.size(2), weight.size(3)))
        gradOutput = gradOutput.view((gradOutput.size(0), group, gradOutput.size(1) // group, gradOutput.size(2),
        ...
        gradOutput = gradOutput.view((gradOutput.size(0), gradOutput.size(1) * gradOutput.size(2), gradOutput.size(3),
                                      gradOutput.size(4), gradOutput.size(5)))
        ...
        gradInput[elt] = deformable_col2im(columns, offset[elt], nInputPlane, inputHeight, inputWidth, kH, kW, padH,
                                           padW, dH, dW, dilationH,
...
def deform_conv_backward_parameters_cuda(
    ...
    gradOutputBuffer = gradOutputBuffer.view(batchSize // im2col_step, nOutputPlane, im2col_step * outputHeight,
                                             outputWidth)
    ...
    input = input.view(batchSize // im2col_step, im2col_step, nInputPlane, inputHeight, inputWidth)
    offset = offset.view(batchSize // im2col_step, im2col_step, deformable_group * 2 * kH * kW, outputHeight,
                         outputWidth)
    ...
    for elt in range(batchSize // im2col_step):
        ...
        columns = columns.view(group, columns.size(0) // group, columns.size(1))
        gradWeight = gradWeight.view(group, gradWeight.size(0) // group, gradWeight.size(1), gradWeight.size(2),
                                     gradWeight.size(3))

        for g in range(group):
            gradWeight[g] = (gradWeight[g].flatten(1) + scale * jt.matmul(gradOutputBuffer[elt, g].flatten(1),
